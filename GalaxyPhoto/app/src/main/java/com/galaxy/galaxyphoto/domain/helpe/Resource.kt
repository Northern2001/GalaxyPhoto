...
sealed class Resource<out T> {
    ...
    data class ErrorList(val exception:Throwable) : Resource<Nothing>()
...
fun <T> Resource<T>.isError(): Boolean = this is Resource.ErrorList

fun <T> Resource<T>.data(): T = (this as Resource.Success).data
fun <T> Resource<T>.listException(): Throwable = (this as Resource.ErrorList).exception
fun <T> Resource<T>.exception(): Throwable =
    (this as Resource.ErrorList).exception ?: Throwable()
